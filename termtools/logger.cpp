...
void Logger::setShowLocation(bool show) {
    showLocation_ = show;
}
...
Color Logger::levelToColor(LogLevel level) const {
    switch (level) {
        case LogLevel::Debug:    return Color(ColorName::BrightBlack);
        case LogLevel::Info:     return Color(ColorName::Green);
        case LogLevel::Warning:  return Color(ColorName::Yellow);
        case LogLevel::Error:    return Color(ColorName::Red);
        case LogLevel::Critical: return Color(255, 0, 0);
        default:                 return Color(ColorName::White);
    }
}
...
std::string Logger::formatMessage(LogLevel level, const std::string& message) const {
    ...
    if (showLevel_) {
        ...
        oss << levelStyle.toAnsi()
            << "[" << std::setw(8) << std::left << levelStr << "] "
            << levelStyle.toAnsiReset();
    }
