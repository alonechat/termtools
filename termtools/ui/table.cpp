...
Table& Table::setZebraStyles(const StyleBuilder& odd, const StyleBuilder& even) {
    zebraOddStyle_ = odd;
...
StyleBuilder Table::getEffectiveRowStyle(int rowIndex, int rowCount, bool isFooter) const {
...
CellStyle Table::getEffectiveCellStyle(int row, int col, const std::string& content, bool isHeader, bool isFooter) const {
    CellStyle result;
...
std::string Table::renderSeparator(const std::vector<int>& widths, bool top, bool bottom, bool beforeFooter) const {
...
std::string Table::renderTitleOrCaption(const std::string& text, int totalWidth) const {
    std::ostringstream oss;
    Text titleText(text);
...
std::string Table::renderRow(const std::vector<Cell>& cells, const std::vector<int>& widths, int rowIndex, bool isHeader, bool isFooter) const {
    std::ostringstream oss;
    BorderStyle bs = getBorderStyle();
    
    if (showBorder_ && showEdge_ && borderType_ != BorderType::None) {
        oss << borderStyle_.toAnsi() << bs.vertical << borderStyle_.toAnsiReset();
    } else if (showBorder_ && borderType_ == BorderType::Markdown) {
        oss << borderStyle_.toAnsi() << bs.vertical << borderStyle_.toAnsiReset();
    }
    
    int rowCount = static_cast<int>(rows_.size());
    StyleBuilder effectiveRowStyle = getEffectiveRowStyle(rowIndex, rowCount, isFooter);
    ...
    for (size_t i = 0; i < widths.size(); ++i) {
        ...
        CellStyle effectiveCellStyle = getEffectiveCellStyle(rowIndex, i, content, isHeader, isFooter);
...
std::string Table::render() const {
    ...
    if (!footerRows_.empty() && showBorder_ && borderType_ != BorderType::None) {
        oss << renderSeparator(widths, false, false, true);
    }
    
    for (size_t i = 0; i < footerRows_.size(); ++i) {
        oss << renderRow(footerRows_[i], widths, static_cast<int>(i), false, true);
    }
